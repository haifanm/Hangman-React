...
class Hangman extends Component {
  ...
  handleGuess(value) {
    let letter = value;
    this.setState((st) => ({
      guessed: st.guessed.add(letter.toLowerCase()),
      mistake:
        st.mistake + (st.answer[1].includes(letter.toLowerCase()) ? 0 : 1),
    }));
  }
  ...
  getListId() {
    var listId = this.state.answer[0];
    return (
      <p className="text-center text-light" style={{ fontSize: "40px" }}>
        {" "}
        {listId ? "Guess the Country ?" : "Guess the Programming Language ?"}
      </p>
    );
  }
  render() {
    ...
    const isWinner = this.guessedWord().join("") === answer[1];
    ...
    return (
      <div className="Hangman">
        ...
        <p className="Hangman-word text-center">
          {!gameOver ? this.guessedWord() : answer[1]}{" "}
        </p>
